...
其实和题目 [Serialize Binary Tree ](/232-serialize-and-deserialize-binary-tree.md) 的思路差不多。也相当于采用 "pre-order"的思想，先visit root，然后从root的最左边的child到最右边的child做DFS。然后每遍历完一个node的child后，则加一个 marker 表示即可。可以参考 [geeksForGeeks ](https://www.geeksforgeeks.org/serialize-deserialize-n-ary-tree/)  
